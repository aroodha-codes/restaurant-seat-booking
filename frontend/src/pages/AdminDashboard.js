...
import { Lock, LogOut, Plus, Trash2, Edit, CheckCircle, XCircle } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const updateMenuItem = async (itemId) => {
    try {
      await axios.put(`${API}/menu/${itemId}`, {
        ...editingItem,
  ...
  return (
    <div className="min-h-screen bg-forest">
      <div className="border-b border-forest-light bg-forest-light">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <Button
            ...
            variant="outline"
            className="border border-gold text-gold hover:bg-gold hover:text-forest transition-colors duration-300 flex items-center gap-2"
          >
            <LogOut size={18} />
            Logout
          </Button>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex gap-4 mb-8 border-b border-forest-light">
          <button
            onClick={() => setActiveTab('bookings')}
            className={`px-6 py-3 font-serif text-lg transition-colors duration-300 ${
              activeTab === 'bookings'
        ...
        {activeTab === 'bookings' ? (
          <div className="space-y-4">
            {bookings.map((booking) => (
              <div
                key={booking.id}
                className="bg-forest-light border border-forest-light p-6 rounded-md hover:border-gold transition-colors duration-300"
              >
                ...
                <div className="grid grid-cols-2 gap-4 text-sage text-sm mb-4">
                  ...
                  {booking.payment_method && (
                    <p>Payment Method: <span className="text-cream">{booking.payment_method}</span></p>
                  )}
                  {booking.utr_number && (
                    <p>UTR Number: <span className="text-cream">{booking.utr_number}</span></p>
                  )}
                </div>
                
                {booking.special_requests && (
                  <p className="text-sage text-sm mb-4">
                    <span className="text-cream">Special Requests:</span> {booking.special_requests}
                  </p>
                )}
              ...
              <form onSubmit={addMenuItem} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <Input
                    ...
                    className="bg-transparent border-b border-forest-light text-cream"
                    required
                  />
                  <Input
                    placeholder="Price"
                ...
                <Textarea
                  ...
                  className="bg-transparent border border-forest-light text-cream"
                  required
                />
                <div className="grid grid-cols-2 gap-4">
                  <select
                    ...
                    className="bg-forest-light border-b border-forest-light text-cream px-0 py-3"
                  >
                ...
                <Button
                  type="submit"
                  className="bg-gold text-forest hover:bg-gold-dark flex items-center gap-2"
                >
                  <Plus size={18} />
                  Add Menu Item
                </Button>
                  ...
                  {editingItem?.id === item.id ? (
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          className="bg-gold text-forest hover:bg-gold-dark"
                        >
                          Save
                        </Button>
                        <Button
                    ...
                    <>
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="px-3 py-1 bg-gold text-forest rounded-full text-xs font-body">
                          {item.category}
                        </span>
                      </div>
