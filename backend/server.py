...
app = FastAPI(title="Lumière Bistro API")
...
@api_router.get("/")
async def root():
    return {"message": "Lumière Bistro API is running"}


# Bookings
@api_router.post("/bookings", response_model=Booking)
async def create_booking(booking: BookingCreate):
    if not booking.name.strip():
        raise HTTPException(status_code=400, detail="Name is required")
    if not booking.phone.strip():
    ...
    return await db.bookings.find_one({"id": booking_dict['id']}, {"_id": 0})
...
async def create_menu_item(item: MenuItemCreate):
    ...
    return await db.menu_items.find_one({"id": item_dict['id']}, {"_id": 0})
...
async def create_order(order: OrderCreate):
    ...
    return await db.orders.find_one({"id": order_dict['id']}, {"_id": 0})
...
async def verify_upi_payment(request: Dict):
    ...
    await db.upi_payments.insert_one({
        "booking_id": booking_id, "utr_number": utr_number,
        "amount": 500.00, "currency": "INR", "payment_method": "UPI",
        "status": "pending_verification", "created_at": datetime.now(timezone.utc).isoformat()
    })
    await db.bookings.update_one({"id": booking_id}, {"$set": {
        "payment_status": "pending_verification", "payment_method": "UPI", "utr_number": utr_number
    }})
    return {"success": True, "message": "UPI payment recorded. Admin will verify shortly.", "booking_id": booking_id}

@api_router.post("/payment/create-checkout")
async def create_payment_checkout(payment_req: PaymentRequest):
    ...
    if not stripe_api_key or stripe_api_key == 'sk_test_placeholder':
    ...
    session = stripe.checkout.Session.create(
        ...
        line_items=[{"price_data": {"currency": "inr", "product_data": {"name": "Restaurant Booking Deposit"}, "unit_amount": 50000}, "quantity": 1}],
        mode="payment",
        success_url=f"{payment_req.origin_url}/payment-success?session_id={{CHECKOUT_SESSION_ID}}",
        cancel_url=f"{payment_req.origin_url}/",
        metadata=metadata
    )
    await db.payment_transactions.insert_one({"session_id": session.id, "booking_id": payment_req.booking_id, "amount": 500.00, "currency": "inr", "payment_status": "pending", "created_at": datetime.now(timezone.utc).isoformat(), "metadata": metadata})
